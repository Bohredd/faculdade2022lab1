...
int definir_menor_quantia(int intervalo,int *vet,int limite)
{
    ...
    if(aux==0)
    {
        printf("A menor quantidade de primos eh %d e foi no intervalo %d ate %d",menor,1+((y-1)*limite),y*limite);
    } else if (aux>=1)
    {
        printf("A menor quantidade de primos eh %d e teve sua primeira aparicao no intervalo %d ate %d",menor,1+((y-1)*intervalo),y*limite);
    }
}
...
int setar_intervalos(int limite,int intervalo)
{
    ...
    int vet[intervalo],menor_quantia;
    ...
    do
    {
        ...
        if(menor_quantia>quantia_primos){
            menor_quantia = quantia_primos;
